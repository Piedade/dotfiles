...
linkConfig() {
    ...
    if [ ! -d "$LOCAL_BIN_DIR" ]; then
        mkdir -p "$LOCAL_BIN_DIR" || {
            echo "Failed to create directory: $LOCAL_BIN_DIR"
            return 1
        }
    else
        echo "$LOCAL_BIN_DIR exists, skipping creation."
    fi

    echo "${YELLOW}Linking dwmblocks scripts to .local/bin...${RC}"
    for file in $GITPATH/dwm/blocks/scripts/*; do
        filename=$(basename "$file") 

        ln -svf "$file" "$LOCAL_BIN_DIR/$filename" || {
            echo "${RED}Failed to create symbolic link for $filename${RC}"
            exit 1
        }
    done
